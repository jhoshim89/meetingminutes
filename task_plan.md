...
- [x] **정확도 검증** ✅ 83.5% (ClovaNote 대비), VAD onset=0.1 최적화
...
### Phase 3: 자동화 & 템플릿 (2-3주) ✅ 완료
- [x] **회의 템플릿 기능 구현** ✅ (Template API, migrations, CRUD 완료)
- [x] **자동 요약 생성 (EXAONE 3.5)** ✅ 한국어 특화, 8개 주제 추출, 환각 없음
...
## Key Questions (답변 완료)
1. **WhisperX 정확도**: ✅ 83.5% 달성 (ClovaNote 대비), VAD onset=0.1 최적화 필수
2. **화자 인식 정확도**: ⏳ 테스트 필요 (pyannote 임베딩 구현 완료)
3. **RAG 검색 품질**: ⏳ 테스트 필요
4. **성능**: ✅ 61분 오디오 → 52초 처리 (69.7x 실시간)
5. **LLM 요약**: ✅ EXAONE 3.5가 한국어에 최적 (Gemma3 환각 심함)
...
- **STT**: WhisperX large-v3-turbo (VAD onset=0.1, 한국어 83.5%)
- **요약**: EXAONE 3.5 (7.8B) - 한국어 특화, 환각 적음
- **검색**: BGE-M3 (예정)
...
## Errors Encountered
1. **PyTorch 2.6+ weights_only 오류**: pyannote VAD 로드 실패
   - 해결: `config.py`에서 `torch.serialization.add_safe_globals()` 패치 적용
   - 반드시 `from config import ...` 먼저 호출

2. **WhisperX 한국어 텍스트 누락 (18%)**: VAD 기본값(onset=0.5)이 너무 높음
   - 해결: `vad_onset=0.1`, `vad_offset=0.1`로 변경 → 83.5%로 개선

3. **Gemma3 환각 문제**: 전혀 다른 내용 요약
   - 해결: EXAONE 3.5 (7.8B)로 교체 → 환각 없이 정확한 요약

---

## Status
**✅ Phase 2-3 완료! E2E 파이프라인 검증 완료 (2025-01-18)**
...
### Phase 6: 회의 예약 기능 (1-2주) 🔄 진행 중
- [x] **DB 스키마 & 모델 생성** ✅
- [x] **Provider & Supabase 서비스 확장** ✅
- [x] **FCM & 스케줄러 서비스 구현 (PWA)** ✅
- [x] **UI 화면 구현 (캘린더, 폼, 상세)** ✅
- [x] **RecorderProvider 연동 & PWA 딥링크** ✅
- [ ] **테스트 & 검증** ⏳ (Firebase 설정 후 테스트 필요)

**테스트 체크리스트**:
- [ ] Firebase 프로젝트 생성 및 VAPID 키 발급
- [ ] index.html, firebase-messaging-sw.js에 Firebase config 입력
- [ ] Supabase Edge Function 배포
- [ ] pg_cron 스케줄러 설정
- [ ] PWA 설치 테스트 (iOS Safari 홈 화면 추가)
- [ ] Push 알림 권한 요청 및 수신 테스트
- [ ] 딥링크 테스트 (알림 탭 → RecorderScreen 이동)
- [ ] 녹음 완료 → appointment 상태 업데이트 확인

**Agent**: frontend-architect (sonnet) + backend-architect (sonnet) + devops-architect (sonnet)
**기간**: 1-2주

**PWA 제한사항**:
- ❌ 백그라운드 녹음 불가 (iOS Safari 제한)
- ✅ Web Push 알림 지원 (iOS 16.4+ PWA 설치 시)
- ⚠️ 수동 녹음: 알림 탭 후 사용자가 직접 녹음 버튼 클릭
...
|-------|------|--------|
| Phase 1 | ✅ 완료 | 100% |
| Phase 2 | ✅ 완료 | 100% (E2E 검증 완료) |
| Phase 3 | ✅ 완료 | 100% (EXAONE 3.5 요약) |
| Phase 4 | ⏳ 대기 중 | 0% (RAG 검색) |
| Phase 5 | ⏳ 대기 중 | 0% |
| Phase 6 | ⏳ 대기 중 | 0% (예약 기능) |
